...
### Added — 综合择时信号（仓位建议）
- 后端 `services/market_sentiment.py`：`_compose_timing` 纯函数（api / beat 共用）——温度 0.5 + 涨跌停强度 0.3 + 北向 0.2 加权合成 0-100 评分与档位（重仓/半仓/轻仓/空仓观望），北向缺失自动重分配权重 0.6/0.4；`get_timing_signal` 全走 Redis 缓存毫秒级
- 后端 `api/market.py` `GET /market/timing-signal` + 收盘 beat 顺带 `publish_event("market.timing")` 推送仓位建议（`EVENT_TYPES` 注册，通知中心可订阅）
