...
**当前版本**: v0.8.38 — 四方向合集（综合择时信号 + AI 投研周报 + 行业热度 + 回测持久化）
...
- [x] **v0.8.2~0.8.22：策略库与短线选股线**
  - 策略库扩到 15 类（KDJ / 网格 / DMI / 吊灯止损 / 双均线量能 / 回踩 / 布林收口 / CCI / VWAP 偏离 / 金字塔海龟…）
  - 回测引擎：交易明细回合配对（MAE/MFE）+ 动量轮动 + 配对交易 + 多周期 + walk-forward OOS + HTML 报告导出
  - 短线选股全链路：4 形态扫描（放量突破 / 均线多头 / 回踩企稳 / 涨停打板）→ 收盘自动推送 → 盯盘标记 → 形态有效性实证 → 打板复盘（连板溢价 / 晋级率）→ 多形态共振 → K 线形态打标
  - 模拟盘：资金账户 + 每日净值快照 + 基准对比；数据健康面板
- [x] **v0.8.23~0.8.35：多因子研究线**
  - 时序因子库 11 因子（动量 / 波动 / 趋势 / 量能 / 反转 / 量价资金）+ z-score 加权选股 + 因子快照缓存（收盘 beat 刷新）
  - 验因子：单因子 IC / 5 档分层回测 + 全因子 IC 横评
  - 组合回测：top N 等权定期调仓 + 参数寻优热力图 + walk-forward 防过拟合 + 报告导出 + 每日自动推送
- [x] **v0.8.36~0.8.38：择时线 + 产出沉淀**
  - 市场情绪：温度计（涨跌停 / 涨跌比 / 赚钱效应）+ 连板梯队 + 北向资金 + 行业热度（后两者接口降级框架）
  - 综合择时信号：三维合成 0-100 仓位建议（重仓 / 半仓 / 轻仓 / 空仓观望）+ 收盘飞书推送
  - AI 投研周报（净值周复盘 + 情绪周走势 + AI 综述 HTML）；组合回测存档 PG + 历史对比
- [ ] Phase 9（暂缓）：实盘 Gateway 接入 —— BaseGateway 契约 + 工厂 + 配置切换已就位；实盘网关（QMT/xtquant）明确暂不做（专注选股 / 择时研究）
- [ ] 待数据积累：情绪择时回测（需情绪温度历史存档满数月）
